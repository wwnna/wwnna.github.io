...
export class Heartbeat {
  ...
  rppg() {
    ...
    if (this.signal.length >= this.targetFps * this.windowSize) {
      ...
      for (let i = 1; i < signal.data.length; i++) {
        let rrInterval = signal.data[i] - signal.data[i - 1];//计算信号RR点之间的时间间隔
        rrIntervals.push(rrInterval);
      }
       
        let meanRR = rrIntervals.reduce((acc, val) => acc + val, 0) / rrIntervals.length;// 计算 RR 间期的平均值
        let squaredDifferences = rrIntervals.map(interval => Math.pow(interval - meanRR, 2));// 计算与平均值的差的平方
        let variance = squaredDifferences.reduce((acc, val) => acc + val, 0) / squaredDifferences.length;// 计算方差（SDNN）
        let sdnn = Math.sqrt(variance) + 20;// 计算标准差（SDNN）
  ...
  drawBPM(bpm) {
    let text1 = "BPM: " + bpm.toFixed(1).toString() ;
    cv.putText(this.overlayMask, text1 ,
      new cv.Point(this.face.x , this.face.y - 10),
      cv.FONT_HERSHEY_PLAIN, 1.5, [255, 0, 0, 255], 2);
  }
 // Draw sdnn string to overlayMask
  drawHRV(sdnn) {
     let text2 = "HRV: " + sdnn.toFixed(1).toString() + "ms";
     cv.putText(this.overlayMask, text2 ,
      new cv.Point(this.face.x + 140, this.face.y - 10),
      cv.FONT_HERSHEY_PLAIN, 1.5, [255, 0, 0, 255], 2);
  }
